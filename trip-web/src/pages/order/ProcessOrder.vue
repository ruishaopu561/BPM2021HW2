...
export default {
  ...
  methods: {
    ...
    address(key) {
      if (key === 15 && this.dataSource[7].status === statuss[2]) {
        this.$message.success("已处理修改请求");
        this.dataSource[7].status = statuss[3]
        return
      }
      ...
      this.$message.success("已处理预约请求");
      if (key === 15 && this.dataSource[7].status === statuss[1]) {
        setTimeout(() => {
          this.$message.success("有订单修改请求");
          this.dataSource[7].status = statuss[2]
          this.dataSource[7].ordertime = "2021-" + "12" + "-" + "30"
        }, 5000)
      }
    },
    finish(key) {
  ...
  mounted() {
    setTimeout(() => {
      this.$message.success("有新的预约请求");
      this.dataSource.push({
        key: 15,
        no: "NO " + 15,
        username: "jjj",
        ordertime: "2021-" + "12" + "-" + "23",
        status: statuss[0],
        type: types[0],
        number: 2,
        description: "这是一段描述",
