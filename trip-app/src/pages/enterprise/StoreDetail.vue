  ...
  <div>
    <a-card style="margin-top: 16px" :bordered="false">
      <detail-list title="企业信息" :col="1">
        <detail-list-item term="企业名称">{{
          detail.username
        }}</detail-list-item>
        <detail-list-item term="联系电话">{{ detail.phone }}</detail-list-item>
        <detail-list-item term="开业时间">{{
          detail.starttime
        }}</detail-list-item>
    ...
    <a-card title="简介">
      <detail-list size="small" :col="1">
        <detail-list-item>
          {{
            detail.username
          }}毗邻南京路步行街，世纪广场对面，酒店闹中取静，既可一步之遥享受南京路的繁华，又可后退一步，坐拥上海繁华街景，且可闲庭信步，打卡人民广场、杜莎夫人蜡像馆、外滩、豫园城隍庙、博物馆、新天地等上海著名景点，隔江眺望陆家嘴、东方明珠、上海中心，领略上海风情万种的魅力。
        </detail-list-item>
      </detail-list>
      ...
      <detail-list size="small" :col="1">
        <detail-list-item>
          {{
            detail.username
          }}定位为中高端轻奢酒店，整体设计风格典雅现代、臻至高贵，荣获2020香港亚太设计师大赛酒店空间（方案类）至高奖项——至尊奖。酒店以不同的“城市温度”为艺术主题，让宾客感受每一座城的温度，书写每一座城的回忆，打造“心之所向，皆有心安之所”的舒适旅居之所。
        </detail-list-item>
      </detail-list>
      <a-divider style="margin: 16px 0" />
      <detail-list size="small" :col="1">
    ...
    <a-card style="margin-top: 24px 0">
      ...
      >
        ...
        <div slot="action" slot-scope="{ text }">
          <a @click="handleOrder(text.key)"> <a-icon type="form" />预订 </a>
        </div>
      </standard-table>
      <a-modal
        title="新增订单"
